...
print("Generating Moons dataset from sklearn...")
X_np, y_np = make_moons(n_samples=1000, noise=0.1, random_state=42)

df = pd.DataFrame(X_np, columns=["x", "y"])
df["label"] = y_np
df.to_csv("data.csv", index=False)
print(f"Saved {len(df)} samples to data.csv")

# 2. Convert to Tensors and 80/20 Train/Test Split
...
# 3. Normalization (Crucial: compute stats on train set, apply to both)
...
# 4. Model Architecture (16 hidden units for crescent curve capacity)
class Net(nn.Module):
    def __init__(self):
        super().__init__()
...
print("Starting training on Moon dataset...")
...
# 7. Save Checkpoint (Weights + Normalization Stats)
checkpoint = {
...
if os.path.exists(demo_dir):
    torch.save(checkpoint, os.path.join(demo_dir, "model.pth"))
    shutil.copy("data.csv", os.path.join(demo_dir, "data.csv"))
    print(f"Synced model.pth and data.csv to {demo_dir}/")
